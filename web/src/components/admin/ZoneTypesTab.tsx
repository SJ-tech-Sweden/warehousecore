...
export function ZoneTypesTab() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm(t('admin.zoneTypes.confirmDelete'))) return;
    try {
      ...
      {editing && (
        <div className="glass rounded-xl p-4 space-y-3 border-2 border-accent-red">
          <p className="text-sm text-gray-400">
            {t('admin.zoneTypes.ledMovedPrefix')}{' '}
            <span className="text-white font-semibold">{t('admin.zoneTypes.ledMovedTarget')}</span>.
          </p>
          <div className="grid grid-cols-2 gap-3">
            <input
              type="text"
              placeholder={t('admin.zoneTypes.keyPlaceholder')}
              value={formData.key ?? ''}
            ...
            <input
              type="text"
              placeholder={t('admin.zoneTypes.labelPlaceholder')}
      ...
      <div className="space-y-2">
        {zoneTypes.map(zt => (
          <div key={zt.id} className="glass rounded-xl p-4 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={() => startEdit(zt)} className="p-2 hover:bg-white/10 rounded-lg text-blue-400" title={t('common.edit')} aria-label={t('common.edit')}>
                <Edit2 className="w-4 h-4" />
              </button>
              <button onClick={() => handleDelete(zt.id)} className="p-2 hover:bg-white/10 rounded-lg text-red-400" title={t('common.delete')} aria-label={t('common.delete')}>
