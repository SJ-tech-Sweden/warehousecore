	...
	if elements, ok := labelData["elements"].([]map[string]interface{}); ok {
		for _, elem := range elements {
			...
			if elemType == "text" {
				...
				// Parse font size (default to 11pt if not specified)
				fontSize := 11.0
				if style != nil {
				...
				// Scale the rendering based on desired font size
				face := basicfont.Face7x13

				// Calculate scale factor based on desired point size
				// At 300 DPI: 11pt = 11/72 * 300 = 46px height
				// Face7x13 is 13px, so scale = 46/13 = 3.5x
				scaleFactor := (fontSize / 72.0 * 300.0) / 13.0

				// Render text at scaled size
				scaledY := y + int(fontSize * 11.8 / 72.0 * 300.0 / 13.0)
				...
				// For better appearance at larger sizes, draw characters with spacing
				if scaleFactor > 1.5 {
					// Draw each character with proper spacing for larger text
					xOffset := x
					for _, ch := range content {
						d.Dot.X = fixed.Int26_6(xOffset * 64)
						d.DrawString(string(ch))
						xOffset += int(7 * scaleFactor) // 7px is the char width of Face7x13
					}
				} else {
