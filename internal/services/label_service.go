...
func NewLabelService() *LabelService {
	...
	if err != nil {
		log.Printf("Warning: Could not load TrueType font: %v", err)
		return &LabelService{}
	}

	font, err := truetype.Parse(fontData)
	if err != nil {
		log.Printf("Warning: Could not parse TrueType font: %v", err)
		return &LabelService{}
	}
		...
		for _, elem := range elements {
			...
			if elemType == "text" {
				...
				if s.defaultFont != nil {
					...
					// Ascent is typically ~0.8 of font size for most fonts
					ascentPixels := fontSizePixels * 0.8

					// Baseline Y = element Y + ascent
					baselineY := y + int(ascentPixels)

					log.Printf("[LABEL DEBUG] Rendering: x=%dpx, baselineY=%dpx, fontSizePx=%.1f, ascent=%.1f",
						x, baselineY, fontSizePixels, ascentPixels)
					...
					log.Printf("[LABEL DEBUG] Text '%s' rendered successfully", content)
				}
